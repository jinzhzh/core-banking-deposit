...
class WithdrawalService:
    ...
    def withdraw(
        ...
        参数:
            account_number: 账户号码
            amount: 取款金额，必须为正数且使用 Decimal 精确计算
            summary: 摘要信息，默认为"现金支取"

        返回:
            dict: 包含交易流水信息的字典

        异常:
            ValueError: 金额不合法、余额不足或账户状态异常
        """
        # 参数校验
        ...
        if account["status"] != "active":
            raise ValueError(f"账户 {account_number} 状态为 {account['status']}，无法取款")

        # 检查可用余额（余额 - 冻结金额）
        frozen_amount = account.get("frozen_amount", Decimal("0"))
        available_balance = account["balance"] - frozen_amount

        if amount > available_balance:
            raise ValueError(
                f"可用余额不足。当前余额: {account['balance']}，"
                f"冻结金额: {frozen_amount}，"
                f"可用余额: {available_balance}，"
                f"取款金额: {amount}"
            )
